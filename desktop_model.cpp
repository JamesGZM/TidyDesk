...
HRESULT MoveOutAndRemove(HWND owner,const std::wstring& folder,const std::wstring& destination){std::error_code ec;bool exists=std::filesystem::exists(folder,ec);if(ec)return E_ACCESSDENIED;if(!exists)return S_OK;std::vector<std::wstring> files;for(auto& entry:std::filesystem::directory_iterator(folder,ec))files.push_back(entry.path().wstring());if(ec)return E_ACCESSDENIED;if(!files.empty()){auto hr=Transfer(owner,files,destination,false);if(FAILED(hr))return hr;}std::filesystem::remove(folder,ec);return ec?HRESULT_FROM_WIN32(ERROR_DIR_NOT_EMPTY):S_OK;}
bool Dissolve(HWND owner,const std::wstring& id){auto boxes=Load();auto found=std::find_if(boxes.begin(),boxes.end(),[&](const Box& b){return b.id==id;});if(found==boxes.end())return false;
 ...
 if(MessageBoxW(owner,prompt.c_str(),L"解散收纳框",MB_OKCANCEL|MB_ICONQUESTION|MB_DEFBUTTON2)!=IDOK)return false;
 auto hr=MoveOutAndRemove(owner,folder,Known(FOLDERID_Desktop).wstring());if(FAILED(hr)){if(hr!=HRESULT_FROM_WIN32(ERROR_CANCELLED))MessageBoxW(owner,L"未完成解散：仍有文件未移出，或原文件夹被占用。收纳框与剩余文件已保留。",L"TidyDesk",MB_OK|MB_ICONERROR);return false;}
 boxes=Load();boxes.erase(std::remove_if(boxes.begin(),boxes.end(),[&](const Box& b){return b.id==id;}),boxes.end());if(!Save(boxes)){MessageBoxW(owner,L"文件已移回桌面，但布局保存失败，请刷新后重试解散。",L"TidyDesk",MB_OK|MB_ICONERROR);return false;}Notify();return true;
}
...
HRESULT Transfer(HWND owner,const std::vector<std::wstring>& paths,const std::wstring& destination,bool executableLinks){
 ...
 size_t queued=0;for(const auto& path:paths){std::filesystem::path src(path);if(_wcsicmp(src.parent_path().c_str(),destination.c_str())==0)continue;
 ...
 }else{ComPtr<IShellItem> item;hr=SHCreateItemFromParsingName(path.c_str(),nullptr,IID_PPV_ARGS(&item));if(FAILED(hr))return hr;hr=op->MoveItem(item.Get(),dest.Get(),nullptr,nullptr);if(FAILED(hr))return hr;++queued;}}
 if(!queued)return S_OK;hr=op->PerformOperations();BOOL aborted=FALSE;op->GetAnyOperationsAborted(&aborted);return aborted?HRESULT_FROM_WIN32(ERROR_CANCELLED):hr;
}
...
int TransferTest(){wchar_t temp[MAX_PATH]{};GetTempPathW(MAX_PATH,temp);auto root=std::filesystem::path(temp)/(L"TidyDesk-transfer-"+std::to_wstring(GetCurrentProcessId()));auto a=root/L"原始",b=root/L"分类";std::filesystem::create_directories(a);std::filesystem::create_directories(b);auto file=a/L"工作 文件.txt",exe=a/L"软件.exe";{std::ofstream f(file);f<<"test payload";}{std::ofstream f(exe);f<<"not executable; shortcut test only";}if(FAILED(Transfer(nullptr,{},L"",false)))return 19;auto hr=Transfer(nullptr,{file.wstring(),exe.wstring()},b.wstring());if(FAILED(hr)){std::printf("Transfer failed: 0x%08lx\n",static_cast<unsigned long>(hr));return 10;}if(std::filesystem::exists(file)||!std::filesystem::exists(b/file.filename())||!std::filesystem::exists(exe)||!std::filesystem::exists(b/L"软件.lnk")){std::printf("Move/shortcut existence invariant failed\n");return 11;}CoInitializeEx(nullptr,COINIT_APARTMENTTHREADED);ComPtr<IShellLinkW> link;CoCreateInstance(CLSID_ShellLink,nullptr,CLSCTX_INPROC_SERVER,IID_PPV_ARGS(&link));ComPtr<IPersistFile> persist;link.As(&persist);persist->Load((b/L"软件.lnk").c_str(),STGM_READ);wchar_t target[32768]{};auto pathResult=link->GetPath(target,32768,nullptr,SLGP_RAWPATH);std::error_code compareError;if(FAILED(pathResult)||!std::filesystem::equivalent(exe,std::filesystem::path(target),compareError)){std::printf("Shortcut target mismatch: hr=%08lx expected=%ls actual=%ls\n",static_cast<unsigned long>(pathResult),exe.c_str(),target);return 12;}persist.Reset();link.Reset();CoUninitialize();if(FAILED(MoveOutAndRemove(nullptr,a.wstring(),b.wstring()))||std::filesystem::exists(a)||!std::filesystem::exists(b/exe.filename()))return 20;auto empty=root/L"empty";std::filesystem::create_directory(empty);if(FAILED(MoveOutAndRemove(nullptr,empty.wstring(),b.wstring()))||std::filesystem::exists(empty))return 21;std::filesystem::remove_all(root);return 0;}
}
