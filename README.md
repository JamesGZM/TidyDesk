...
权限功能的支持范围与历史修复见 [PERMISSIONS.md](PERMISSIONS.md) 和 [SECURITY.md](SECURITY.md)。
...
- 系统快捷方式箭头隐藏/显示，点击应用后请求 Windows 授权；先备份原设置，写入后读取确认，刷新由用户控制。
- 权限操作只启动系统目录中的 Windows reg.exe，固定修改箭头覆盖项；不启动 TidyDesk 提权助手或安装服务。设置仍使用显式应用。
...
0.4.6：快捷方式图标读取原始资源，避开异常快捷方式覆盖缓存；完整窗口接收右键，文件菜单附收纳框操作；展开遮罩覆盖任务栏，调整四边留白。

0.4.7：恢复原网格列数和位置规则，补回设置标题栏图标；收纳格与文件夹同步重命名，禁止重名与覆盖。调整展开态按钮内缩，加入遵循系统动画开关的 160ms 过渡。箭头覆盖使用新的近透明 alpha 资源，系统缓存的实际效果仍须在目标 Explorer 验证。

0.4.9：撤回不安全的提权引导，移除服务安装和箭头提权助手；收纳格使用连续尺寸过渡、固定图标缓存与遮罩渐变，支持中途反向，完成后释放缓存和定时器。原网格、图标尺寸和最终布局不变。

0.4.10：常规页移除历史维护提示；图标页仅在需要时提供恢复入口。收纳格内拖拽显示插入位置，支持跨行、多选、边缘滚动，松手立即显示并持久保存顺序；内部排序保持当前展开状态，取消不改变顺序。

0.4.11：修复收纳格焦点和选中状态：切换格子或点击外部清理旧选中，保留拖拽与菜单操作中的选择；单击已选项目恢复单选，多选拖动仍保留整组。

0.4.12：恢复快捷方式箭头开关与显式应用，管理员操作使用 System32/reg.exe 的固定注册表命令；备份保留原值类型，取消授权可重试，恢复兼容旧版本备份。透明覆盖资源补齐全透明 AND 遮罩并使用稳定数据目录，避免升级卸载后路径丢失；Explorer 缓存仍需实际桌面验证。
