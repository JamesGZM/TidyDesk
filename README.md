...
0.2.1 实验版本：已实现原生托盘程序和 XAML 透明后端，目标为 Windows 11 22H2 及以后的 x64 系统。在一台 Windows 11 25H2（26200.9168）机器上已完成透明应用、正常退出恢复、再次启动测试。用户确认透明效果、图标及点击正常。优化版主进程在 45 秒采样末端工作集为 **9.01 MiB**、私有内存 **1.47 MiB**，该区间 CPU 累计时间没有增加。此结果不是所有设备或系统版本的性能保证，详见 [验证记录](VALIDATION.md)。

无需编译：到 [Releases](https://github.com/JamesGZM/LiteTaskbar/releases) 下载实验版 ZIP。尚未覆盖多显示器、睡眠唤醒及长时间使用，不作为稳定版发布。
