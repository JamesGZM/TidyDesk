...
桌面空白处右键 → 新建收纳框（Windows 11 可能在“显示更多选项”内）。设置页只启停模块和管理已有收纳框。拖边缘调整大小，拖标题移动；图标不随框缩小。滚轮浏览，无滚动条和数量提示。右上角展开/收起，展开时点击外部或 Esc 返回原尺寸。拖入停留 650ms 展开，拖出时撤掉遮罩。框内空白处右键管理；文件上右键使用 Windows 文件菜单。解散时选择移回桌面或保留原文件夹，不删除文件。
