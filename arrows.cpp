...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR command,int){if(wcscmp(command,L"--test-overlay")==0){auto p=std::filesystem::temp_directory_path()/(L"TidyDesk-overlay-"+std::to_wstring(GetCurrentProcessId())+L".ico");bool ok=overlay::Write(p)&&overlay::Verify(p);std::error_code ec;std::filesystem::remove(p,ec);return ok?0:20;}if(wcscmp(command,L"--test-backup")==0){
