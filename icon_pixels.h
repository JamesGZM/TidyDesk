...
// CreateIconIndirect accepts straight RGB and prepares the alpha icon internally.
inline int IconPixelsTest(){BITMAPINFO info{};info.bmiHeader.biSize=sizeof(BITMAPINFOHEADER);info.bmiHeader.biWidth=16;info.bmiHeader.biHeight=-16;info.bmiHeader.biPlanes=1;info.bmiHeader.biBitCount=32;void* bits=nullptr;auto color=CreateDIBSection(nullptr,&info,DIB_RGB_COLORS,&bits,nullptr,0);BYTE mask[32]{};auto monochrome=CreateBitmap(16,16,1,1,mask);if(!color||!monochrome){if(color)DeleteObject(color);if(monochrome)DeleteObject(monochrome);return 1;}auto p=static_cast<DWORD*>(bits);std::fill_n(p,256,0U);p[0]=0xff000000;p[1]=0xffff0000;p[2]=0x80ff0000;ICONINFO iconInfo{TRUE,0,0,monochrome,color};auto icon=CreateIconIndirect(&iconInfo);auto pixels=IconPixels(icon,16);if(icon)DestroyIcon(icon);DeleteObject(color);DeleteObject(monochrome);if(pixels.size()!=1024)return 2;if(pixels[3]!=255||pixels[2]!=0||pixels[7]!=255||pixels[6]!=255||pixels[11]<126||pixels[11]>130||pixels[10]<126||pixels[10]>130||pixels[15]!=0)return 3;return 0;}
}
