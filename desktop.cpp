...
struct Frame {
 ...
 std::set<int> selected;int scroll=0;bool hover=false,buttonHover=false,dropHover=false,large=false,pressed=false,dragging=false,painted=false;
 ...
 void Scan(){if(busy.exchange(true)){again=true;return;}if(loader.joinable())loader.join();auto path=box.path;auto target=window;auto stop=cancel;
  loader=std::thread([path,target,stop](){CoInitializeEx(nullptr,COINIT_APARTMENTTHREADED);auto batch=std::make_unique<Batch>();std::error_code ec;
   for(const auto& entry:std::filesystem::directory_iterator(path,ec)){if(WaitForSingleObject(stop,0)==WAIT_OBJECT_0)break;auto attrs=GetFileAttributesW(entry.path().c_str());if(attrs==INVALID_FILE_ATTRIBUTES||(attrs&(FILE_ATTRIBUTE_HIDDEN|FILE_ATTRIBUTE_SYSTEM)))continue;
    Item i;i.path=entry.path().wstring();i.name=entry.path().filename().wstring();i.label=i.name;if(_wcsicmp(entry.path().extension().c_str(),L".lnk")==0||_wcsicmp(entry.path().extension().c_str(),L".url")==0)i.label=entry.path().stem().wstring();SHFILEINFOW info{};
    if(batch->items.size()<512&&SHGetFileInfoW(i.path.c_str(),0,&info,sizeof(info),SHGFI_ICON|SHGFI_LARGEICON)){i.pixels=desk::IconPixels(info.hIcon,64);DestroyIcon(info.hIcon);}
