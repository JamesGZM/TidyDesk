...
struct Frame {
 ...
 std::set<int> selected;int scroll=0;bool hover=false,dropHover=false,large=false,pressed=false,dragging=false;
...
LRESULT CALLBACK FrameProc(HWND w,UINT msg,WPARAM wp,LPARAM lp){auto f=reinterpret_cast<Frame*>(GetWindowLongPtrW(w,GWLP_USERDATA));if(msg==WM_NCCREATE){f=static_cast<Frame*>(reinterpret_cast<CREATESTRUCTW*>(lp)->lpCreateParams);f->window=w;SetWindowLongPtrW(w,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(f));}if(!f)return DefWindowProcW(w,msg,wp,lp);
 if(f->menu3){LRESULT result=0;if(SUCCEEDED(f->menu3->HandleMenuMsg2(msg,wp,lp,&result)))return result;}else if(f->menu2&&(msg==WM_INITMENUPOPUP||msg==WM_DRAWITEM||msg==WM_MEASUREITEM)){f->menu2->HandleMenuMsg(msg,wp,lp);return 0;}
 ...
 case WM_MOUSEMOVE:{POINT p{GET_X_LPARAM(lp),GET_Y_LPARAM(lp)};if(!f->hover){f->hover=true;TRACKMOUSEEVENT track{sizeof(track),TME_LEAVE,w,0};TrackMouseEvent(&track);Paint(f);}if(f->pressed&&(abs(p.x-f->press.x)>GetSystemMetrics(SM_CXDRAG)||abs(p.y-f->press.y)>GetSystemMetrics(SM_CYDRAG))){f->pressed=false;ReleaseCapture();DragOut(f);}return 0;}
