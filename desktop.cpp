...
struct Frame {
 ...
 std::set<int> selected;int scroll=0;bool hover=false,buttonHover=false,dropHover=false,large=false,pressed=false,dragging=false,painted=false,hitSurface=false;
...
void Paint(Frame* f){if(!f->window||!IsWindow(f->window))return;RECT r{};GetClientRect(f->window,&r);int width=r.right,height=r.bottom;if(width<1||height<1)return;
 ...
 f->hitSurface=true;for(size_t pixel=0;pixel<static_cast<size_t>(width)*height;++pixel)if((static_cast<DWORD*>(pixels)[pixel]>>24)==0){f->hitSurface=false;break;}
 POINT origin{};SIZE size{width,height};BLENDFUNCTION blend{AC_SRC_OVER,0,255,AC_SRC_ALPHA};f->painted=UpdateLayeredWindow(f->window,screen,nullptr,&size,dc,&origin,0,&blend,ULW_ALPHA)!=FALSE;SelectObject(dc,old);DeleteObject(dib);DeleteDC(dc);ReleaseDC(nullptr,screen);
}
...
void Expand(Frame* f,bool value){if(value==f->large)return;if(value&&expanded)Expand(expanded,false);
 if(value){GetWindowRect(f->window,&f->saved);f->large=true;expanded=f;f->box.collapsed=false;MONITORINFO monitor{sizeof(monitor)};GetMonitorInfoW(MonitorFromWindow(f->window,MONITOR_DEFAULTTONEAREST),&monitor);auto area=monitor.rcWork;auto backdrop=monitor.rcMonitor;
  ...
  shade=CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_LAYERED,L"TidyDesk.Desktop.Shade",L"",WS_POPUP,backdrop.left,backdrop.top,backdrop.right-backdrop.left,backdrop.bottom-backdrop.top,controller,nullptr,GetModuleHandleW(nullptr),nullptr);
  if(shade){SetLayeredWindowAttributes(shade,0,60,LWA_ALPHA);HRGN region=CreateRectRgn(0,0,backdrop.right-backdrop.left,backdrop.bottom-backdrop.top);HRGN hole=CreateRoundRectRgn(x-backdrop.left,y-backdrop.top,x-backdrop.left+width,y-backdrop.top+height,f->D(32),f->D(32));CombineRgn(region,region,hole,RGN_DIFF);DeleteObject(hole);if(!SetWindowRgn(shade,region,FALSE))DeleteObject(region);SetWindowPos(shade,HWND_TOPMOST,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE|SWP_SHOWWINDOW);}
  ShowWindow(f->window,SW_HIDE);SetParent(f->window,nullptr);SetWindowLongPtrW(f->window,GWL_STYLE,WS_POPUP|WS_VISIBLE);SetWindowLongPtrW(f->window,GWLP_HWNDPARENT,reinterpret_cast<LONG_PTR>(controller));SetWindowPos(f->window,HWND_TOPMOST,x,y,width,height,SWP_FRAMECHANGED|SWP_SHOWWINDOW);SetForegroundWindow(f->window);SetFocus(f->window);
 }else{HideShade();expanded=nullptr;f->large=false;ShowWindow(f->window,SW_HIDE);SetWindowLongPtrW(f->window,GWLP_HWNDPARENT,0);SetWindowLongPtrW(f->window,GWL_STYLE,WS_POPUP|WS_VISIBLE|WS_THICKFRAME);SetWindowLongPtrW(f->window,GWLP_HWNDPARENT,reinterpret_cast<LONG_PTR>(desktop));POINT p{f->box.x,f->box.y};SetWindowPos(f->window,HWND_NOTOPMOST,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);SetWindowPos(f->window,HWND_BOTTOM,p.x,p.y,f->box.w,f->box.h,SWP_NOACTIVATE|SWP_FRAMECHANGED|SWP_SHOWWINDOW);}
 Paint(f);
}
...
void MenuCommand(Frame* f,int cmd){if(cmd==1){if(Rename(f->window,f->box.name)){Persist();Paint(f);}}else if(cmd==2)Expand(f,!f->large);else if(cmd==3){f->box.locked=!f->box.locked;Persist();}else if(cmd==4)ShellExecuteW(f->window,L"open",f->box.path.c_str(),nullptr,nullptr,SW_SHOWNORMAL);else if(cmd==5)Dissolve(f);else if(cmd>=20&&cmd<=22){f->box.size=static_cast<unsigned>(32+(cmd-20)*16);Persist();Paint(f);}}
LRESULT CALLBACK FrameProc(HWND w,UINT msg,WPARAM wp,LPARAM lp){auto f=reinterpret_cast<Frame*>(GetWindowLongPtrW(w,GWLP_USERDATA));if(msg==WM_NCCREATE){f=static_cast<Frame*>(reinterpret_cast<CREATESTRUCTW*>(lp)->lpCreateParams);f->window=w;SetWindowLongPtrW(w,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(f));}if(!f)return DefWindowProcW(w,msg,wp,lp);
 ...
 case WM_MOUSEACTIVATE:if(!f->large)return MA_NOACTIVATE;break;
...
void Rebuild(){if(interaction){SetTimer(controller,1,250,nullptr);return;}rebuilding=true;HideShade();expanded=nullptr;frames.clear();desktop=nullptr;EnumWindows(FindDesktop,reinterpret_cast<LPARAM>(&desktop));if(!desktop)desktop=FindWindowW(L"Progman",nullptr);if(!desktop){rebuilding=false;SetTimer(controller,1,1500,nullptr);return;}
 for(auto& box:desk::Load()){auto f=std::make_unique<Frame>();f->box=box;f->box.collapsed=false;RECT r{box.x,box.y,box.x+box.w,box.y+box.h};MONITORINFO monitor{sizeof(monitor)};GetMonitorInfoW(MonitorFromRect(&r,MONITOR_DEFAULTTONEAREST),&monitor);f->box.x=std::clamp<LONG>(box.x,monitor.rcWork.left,(std::max)(monitor.rcWork.left,monitor.rcWork.right-box.w));f->box.y=std::clamp<LONG>(box.y,monitor.rcWork.top,(std::max)(monitor.rcWork.top,monitor.rcWork.bottom-box.h));POINT p{f->box.x,f->box.y};
  CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_LAYERED,FrameClass,box.name.c_str(),WS_POPUP|WS_VISIBLE|WS_THICKFRAME,p.x,p.y,box.w,box.h,desktop,nullptr,GetModuleHandleW(nullptr),f.get());if(f->window){SetWindowPos(f->window,HWND_BOTTOM,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE|SWP_SHOWWINDOW|SWP_FRAMECHANGED);Paint(f.get());PostMessageW(f->window,WM_APP+8,0,0);f->Scan();f->Watch();frames.push_back(std::move(f));}}
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR command,int){
 ...
 if(testFrame){int result=0;HWND parent=CreateWindowExW(WS_EX_TOOLWINDOW,wc.lpszClassName,L"Test owner",WS_POPUP,0,0,800,600,nullptr,nullptr,instance,nullptr);{Frame f;f.box.name=L"默认收纳格";CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_LAYERED,FrameClass,f.box.name.c_str(),WS_POPUP|WS_VISIBLE|WS_THICKFRAME,40,40,380,290,parent,nullptr,instance,&f);if(f.window){Paint(&f);if(!IsWindowVisible(f.window)||!f.painted||!f.hitSurface||(GetWindowLongPtrW(f.window,GWL_STYLE)&WS_CHILD))result=21;Expand(&f,true);RECT actual{};MONITORINFO monitor{sizeof(monitor)};GetMonitorInfoW(MonitorFromWindow(f.window,MONITOR_DEFAULTTONEAREST),&monitor);if(!shade||!GetWindowRect(shade,&actual)||!EqualRect(&actual,&monitor.rcMonitor)||!(GetWindowLongPtrW(shade,GWL_EXSTYLE)&WS_EX_TOPMOST))result=22;Expand(&f,false);if(GetWindowLongPtrW(f.window,GWL_EXSTYLE)&WS_EX_TOPMOST)result=23;}else result=20;}DestroyWindow(parent);GdiplusShutdown(token);OleUninitialize();CloseHandle(single);return result;}
