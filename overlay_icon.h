...
#include <vector>
namespace overlay {
// Explicit AND=1 / XOR=0 keeps the destination unchanged even when a Shell
// rendering path ignores alpha. A PNG-only all-zero alpha image is insufficient.
inline bool Write(const std::filesystem::path& path) {
    ...
    for(auto size:sizes){dword(40);dword(size);dword(size*2);word(1);word(32);dword(0);dword(size*size*4);dword(0);dword(0);dword(0);dword(0);data.insert(data.end(),size*size*4,0);data.insert(data.end(),((size+31)/32)*4*size,255);}
